...
class VirtualSurgeEngine {
        ...
        switch (eventCode) {
            case 'GOAL':
                this.destinations.forEach(node => {
                    if (node.type === 'food')
                        this.surgeModifiers[node.id] = Math.min(50, (this.surgeModifiers[node.id] || 0) + 15);
                });
                break;
            case 'HALFTIME':
                ...
                this.destinations.forEach(node => {
                    if (node.type === 'restroom')
                        this.surgeModifiers[node.id] = Math.min(50, (this.surgeModifiers[node.id] || 0) + 25);
                });
                break;
            case 'END_MATCH':
                this.simState = 'peak';
    ...
    tick() {
        ...
        this.destinations.forEach(node => {
            ...
            let multiplier = 1.0;
            if (this.simState === 'halftime')
                multiplier = 1.5;
            if (this.simState === 'peak')
                multiplier = 2.0;
